...
function loadCategories(){
    ...
    fetch(categoriesUrl).then(response => {
        ...
        for (i in categoriesList) {
            ...
            categoryNode.setAttribute("id",categoriesList[i].CATEGORY_NAME);
...
function filterSelectedDate(){
    ...
    for (i = 0; i < eventsList.length; i++) {
      ...
      filteredCategoryNode.innerText = `Category : ${filteredCategory}`;
      filterSectionNode.style.display = "block";
   
}

function filterSelectedCategory(id){
    //alert(id);
    //categoryNode.classList.add('home-page-category-selected');
    var categoryList = document.getElementById("event-category-list");
    for (i in categoriesList) {
